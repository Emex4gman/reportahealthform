...
class FacilityClass {
  constructor(obj) {
    ...
    this.fac_type = fac_types.filter(
      (fac_type) => fac_type.id === obj.fac_type
    )[0].fac_type;
    ...
    this.lganame = lgas.filter(
      (lga) => lga.state_id === obj.statename && lga.lga_id === obj.lganame
    )[0].lga;
    this.statename = states.filter(
      (state) => state.id === obj.statename
    )[0].state;
    this.ownership = ownershipList.filter(
      (ownership) => ownership.id === obj.ownership
    )[0].value;
    this.facility_level = obj.facility_level;
    this.services = obj.services.toString().replace(RegExp(/,/gi), ", ");
    this.daysOfOperations = obj.daysOfOperations
      .toString()
      .replace(RegExp(/,/gi), ", ");
    this.specilizations = obj.specilizations
      .toString()
      .replace(RegExp(/,/gi), ", ");
  }
}
